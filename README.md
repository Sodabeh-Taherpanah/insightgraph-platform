...
- **LLM Service:** FastAPI (Python) - Week 6 sidecar
...
### Option 1: Development Mode (Week 6)

```bash
# 1. Start all services with the startup script
chmod +x start-week6.sh
./start-week6.sh
````

This will start:

- FastAPI LLM service on port 8000
- Node.js backend on port 3001
- Frontend on port 3000 (start separately)

### Option 2: Docker Compose

```bash
docker-compose -f docker-compose.week6.yml up
```
...
2. **Start FastAPI LLM Service:**

See `llm-service/README.md` for full service setup. Quick start (from project root):

```bash
cd llm-service
cp .env.example .env
# Install deps and run (recommended inside a venv)
pip install -r requirements.txt
